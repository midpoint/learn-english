...
/* ---------- 我的词库: 浏览器本地导入, 不经过服务器 ----------
   支持四种格式: 本项目 JSON / Qwerty Learner 词典 JSON([{name,trans,usphone}]) / 两列文本(单词<Tab>释义) / PTE 机经文本(编号+句子+意群划分+翻译) */
...
function parseDeckFile(name,text){
  ...
  if(/^\s*[\[{]/.test(text)){
    ...
    if(Array.isArray(j)){ // Qwerty Learner 词典
...
function renderMyDecks(){
  ...
  box.innerHTML=list.length?list.map(d=>`<div><span>${esc(d.name)} <small>${d.items.length} 条 · ${d.kind==='sentence'?'句子':'单词'}</small></span><span><button data-exp="${d.id}">导出</button> <button data-del="${d.id}">删除</button></span></div>`).join(''):'<div class="empty">还没有导入的词库。设置里「导入词库」支持本项目 JSON、Qwerty Learner 词典、两列文本、PTE 机经文本。</div>';
